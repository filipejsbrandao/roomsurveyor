...
namespace RoomSurveyor
{
    ...
    public sealed class CircularLinkedList<T> : ICollection<T>, IEnumerable<T>
    {
        ...
        /// <summary>
        /// Gets Tail node (the last node of the list). Returns NULL if no tail node found
        /// </summary>
        public Node<T> Tail { get { return tail; } }

        /// <summary>
        /// Gets the head node (the first node of the list). Returns NULL if no node found
        /// </summary>
        ...
        /// <returns><see cref="Node<typeparamref name="T"/>"/> instance or NULL</returns>
        ...
        {
            // if the tail is being removed and is also the head of the list then
            // the value will remain on the list and count will be 0
