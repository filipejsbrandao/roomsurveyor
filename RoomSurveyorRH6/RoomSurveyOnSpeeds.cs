...
namespace RoomSurveyorRH6
{
    ...
    {
        public RoomSurveyOnSpeeds()
          : base("RoomSurvey5OnSpeeds", "RS5_OS",
            "A test component that solves multiple ogon to polygon transformations",
            ...
            for(int i = 0; i < ogons.Count; i++)
            {
                ...
                do
                {
                    ...
                    else
                    {
                        ...
                        if (RoomSurvey5.IsValidDiagonal(new Line(poly[from], poly[to]), poly))
                        {
        ...
        public Polyline RoomSurvey(Polyline poly, List<double> lengths, List<double> diagonals, out List<string> outText, out List<Line> diagLines)
        {
            ...
            else
            {
                ...
                orderedPoints.AddRange(RoomSurvey5.OrderByAngle(poly));
                ...
                if (IsPolyOrtho(poly))
                {
                    orderedDiagonals.AddRange(RoomSurvey5.DiagonalOrder(diagMatrix));
                }
                else
                {
                    orderedDiagonals.AddRange(RoomSurvey5.DiagonalOrder(diagMatrix, orderedPoints));
                    orderedDiagonals.AddRange(RoomSurvey5.DiagonalOrder(diagMatrix));
                ...
                {//create a diagonal using the diagonal vector for the user to see and a text intruction indicating the action
                    ...
                    {//if the user has supplied a diagonal add it to the LowerMatrix
                        ...
                        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                        if (RoomSurvey5.InternalTriangle(isTriVec.ToArray(), i, j, matrixSize, out bool itoj) >= 0 && diagonals[c] > 0)
                        {
                            int P1 = RoomSurvey5.InternalTriangle(isTriVec.ToArray(), i, j, matrixSize, out itoj);
                            int P0 = (itoj) ? i : j;
                            int P2 = (itoj) ? j : i;
                            double isleft = IsLeft(poly[P0], poly[P1], poly[P2]);
                            if (!RoomSurvey5.Triangulate(P0, P1, P2, itoj, diagonals[c], isTriVec, polyVec, isleft))
                            {
                        ...
                        int storedDiagonal = RoomSurvey5.StoredDiagonal(diagMatrix, isTriVec, out itoj);
                        if (storedDiagonal > 0)
                        {
                            while (storedDiagonal > 0 && RoomSurvey5.Count0(isTriVec.ToArray(), matrixSize - 1, matrixSize, matrixSize) > 1)
                            {
                                ...
                                int Pone = RoomSurvey5.InternalTriangle(isTriVec.ToArray(), i, j, matrixSize, out itoj);
                                int Pzero = (itoj) ? i : j;
                                int Ptwo = (itoj) ? j : i;
                                double isL = IsLeft(poly[Pzero], poly[Pone], poly[Ptwo]);
                                if (!RoomSurvey5.Triangulate(Pzero, Pone, Ptwo, itoj, diagMatrix[j, i], isTriVec, polyVec, isL))
                                {
                                ...
                                if (itoj) { ijIsTri = true; } else { jiIsTri = true; }
                                storedDiagonal = RoomSurvey5.StoredDiagonal(diagMatrix, isTriVec, out itoj);
                            }
                        }
                        ...
                        if (ijIsTri && !jiIsTri || !ijIsTri && jiIsTri || diagonals[c] < 0 || !ijIsTri && !jiIsTri)
                        {
                            ...
                            {
                                RoomSurvey5.RemoveDiagonals(ijIsTri, jiIsTri, i, j, diagMatrix, orderedDiagonals);
                            }
                            //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                            //--------101 PATTERN-------------
                            ...
                            //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                            int pattern101 = RoomSurvey5.Sequence101(isTriVec.ToArray(), isTriVec.Count);//HERE ADD CHECKS FOR -1 value
                            if (pattern101 >= 0)
                            {
                                int center = pattern101;
                                List<int> diagonals101 = new List<int>();
                                diagonals101.AddRange(RoomSurvey5.DiagonalOrder101(center, diagMatrix));
                                k = RoomSurvey5.StoredDiagonal(diagMatrix, diagonals101);//returns the k index of the stored diagonal on the lower matrix or -1 if none found
                                if (k < 0)
                            ...
                            //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                            int P0 = RoomSurvey5.Pattern000(isTriVec.ToArray(), isTriVec.Count, out int P2, out int P1);
                            if (P0 >= 0)
                            {
                                ...
                                if (!RoomSurvey5.IsValidTriangle(p0p1.Length, p1p2.Length, p2p0.Length, out double diff))
                                {
                                    goto Skip;
                                }
                                Vector3d newP1P2 = RoomSurvey5.NextVector(Point3d.Origin, p0p1, p0p1.Length, p1p2.Length, p2p0.Length);
                                ...
                                newP1P2.Reverse();
                                double angle1 = RoomSurvey5.AngleBetweenVectors(p1p2, newP1P2, false);
                                for (int n = P1; n < P2; n++)
                                {
                                ...
                                newP1P2.Reverse();
                                p1p2 = newP1P2;
                                Vector3d newP2P0 = RoomSurvey5.NextVector(Point3d.Origin, p1p2, p1p2.Length, p2p0.Length, p0p1.Length);
                                ...
                                newP2P0.Reverse();
                                angle1 = RoomSurvey5.AngleBetweenVectors(p2p0, newP2P0, false);
                    ...
                    Skip:
                        ...
                        //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                        int startP00 = RoomSurvey5.Pattern00(isTriVec.ToArray(), isTriVec.Count, out int endP00);
                        if (startP00 >= 0)
                        {
                            ...
                            double angle = RoomSurvey5.AngleBetweenVectors(ij, ji, false);
                        ...
                        //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                        if (RoomSurvey5.Count0(isTriVec.ToArray(), matrixSize - 1, matrixSize, matrixSize) <= 1)
                        {
                            error = ClosingError(polyVec) * 1000;
                            rebuiltPoly = RebuildPoly(poly, polyVec);
                            outText.Add("The Polygon is closed with a " + error + " mm error");
